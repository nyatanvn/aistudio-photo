...
interface StyleSelectorProps {
  ...
  onSelect: (option: ImageStyle) => void;
}

export const StyleSelector: React.FC<StyleSelectorProps> = ({ options, selectedOption, onSelect }) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {options.map((option) => (
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-1">{option.prompt}</p>
        </button>
      ))}
    </div>
  );
